...
/// Walks from Splash through sign-in as an already provisioned admin. Splash's
...
Future<void> _signInToAdmin(WidgetTester tester) async {
  await tester.pump();
...
void main() {
  ...
  testWidgets('approving an applicant removes them from the pending list', (
    tester,
  ) async {
  ...
  testWidgets(
    'rejecting an applicant also removes them from the pending list',
    (tester) async {
      await tester.pumpWidget(_harness(const AdminDashboardScreen()));
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'navigating back without a decision keeps the applicant pending',
    (tester) async {
      await tester.pumpWidget(_harness(const AdminDashboardScreen()));
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'Providers tab loads (or falls back gracefully without a live Firebase app)',
    (tester) async {
      await tester.pumpWidget(const AtYourServiceApp());
      await _signInToAdmin(tester);
  ...
  testWidgets(
    'Admin renders the desktop sidebar shell (not the mobile bottom nav) at a wide viewport',
    (tester) async {
      ...
      expect(
        find.text('Operational snapshot · updated just now'),
        findsOneWidget,
      );
  ...
  testWidgets(
    'desktop Catalog tab loads (or falls back gracefully without a live Firebase app)',
    (tester) async {
      await tester.binding.setSurfaceSize(const Size(1200, 900));
      addTearDown(() => tester.binding.setSurfaceSize(null));
  ...
  testWidgets(
    'desktop Team & Roles tab loads (or falls back gracefully without a live Firebase app), with no fabricated columns',
    (tester) async {
      await tester.binding.setSurfaceSize(const Size(1200, 900));
      addTearDown(() => tester.binding.setSurfaceSize(null));
  ...
  testWidgets(
    'desktop Disputes & Support tab loads (or falls back gracefully without a live Firebase app)',
    (tester) async {
      await tester.binding.setSurfaceSize(const Size(1200, 900));
      addTearDown(() => tester.binding.setSurfaceSize(null));
  ...
  testWidgets(
    'resizing below the desktop breakpoint while Admin is open falls back to the mobile bottom nav',
    (tester) async {
      await tester.binding.setSurfaceSize(const Size(1200, 900));
      addTearDown(() => tester.binding.setSurfaceSize(null));
  ...
  testWidgets('the desktop sidebar theme toggle flips the app-wide theme', (
    tester,
  ) async {
