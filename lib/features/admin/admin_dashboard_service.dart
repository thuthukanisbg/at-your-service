...
Future<List<AdminStat>> fetchAdminStats() async {
  ...
  return _statsFromBookings(bookingsSnapshot.docs);
}

Stream<List<AdminStat>> watchAdminStats() {
  try {
    return FirebaseFirestore.instance
        .collection('bookings')
        .snapshots()
        .asyncMap((snapshot) => _statsFromBookings(snapshot.docs));
  } catch (error, stack) {
    return Stream.error(error, stack);
  }
}

Future<List<AdminStat>> _statsFromBookings(
  List<QueryDocumentSnapshot<Map<String, dynamic>>> bookings,
) async {
  var revenue = 0.0;
  for (final doc in bookings) {
    if (doc.data()['status'] != 'completed') continue;
  ...
  return [
    AdminStat(value: '${bookings.length}', label: 'Total Bookings'),
...
Future<List<AdminApplicant>> fetchPendingApplicants() async {
  ...
  for (var i = 0; i < snapshot.docs.length; i++) {
    ...
    final role =
        ...
        (selectedServices is List && selectedServices.isNotEmpty
