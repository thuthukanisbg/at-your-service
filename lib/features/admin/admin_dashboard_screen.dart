...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  late final Stream<List<AdminStat>> _statsStream = watchAdminStats();
  ...
  Future<void> _review(AdminApplicant applicant) async {
    final decision = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (_) => AdminReviewScreen(applicant: applicant),
      ),
    );
    if (decision != null) {
        ...
        child: ListView(
          ...
          children: [
            ...
            Padding(
              ...
              child: StreamBuilder<List<AdminStat>>(
                stream: _statsStream,
                    ...
                    child: Row(
                      ...
                      children: [
                        for (var i = 0; i < revenueBars.length; i++) ...[
                          Expanded(
                            child: _RevenueBarColumn(bar: revenueBars[i]),
                          ),
                          if (i != revenueBars.length - 1)
            ...
            if (_pending == null)
              ...
              for (final applicant in _pending!)
                _PendingApplicantCard(
                  applicant: applicant,
                  onTap: () => _review(applicant),
                ),
          ],
        ),
      ),
